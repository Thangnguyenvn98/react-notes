...
const Notes = ({notes}) => {
  ...
  return (
    <section >
    <header className='flex justify-between items-center pl-5 pr-5 pb-4'>
    {!search && <h2 className='text-white'>My Notes</h2>}
    ...
    <div className="grid grid-cols-2 gap-4 ">
        {filteredNotes.length === 0 &&<p className='text-white absolute left-20 top-1/2'>No notes found. </p>}
        {
          filteredNotes.map(note => <NoteItem key={note.id} note={note}/>)
        }
    </div>
   <Link to='/create-note' className='text-white shadow-sm p-3 bg-gray-500 rounded-xl bottom-16 right-28 fixed sm:top-8 h-10' >
   <GrAddCircle  />
   </Link>
  </section>
